...
def _get_ml_config_value(env, key):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"mlops-poc have been deployed to {env} (see mlops-poc"
...
def get_model_name(env, test_mode=False):
    ...
    if env == "dev" or test_mode:
        ...
        return f"mlops-poc-model{resource_name_suffix}"
    else:
        # Read ml model name from mlops-poc/terraform
        return _get_ml_config_value(env, "mlops-poc_model_name")
