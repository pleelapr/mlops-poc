...
* Python unit tests for feature computation modules in `mlops-poc/tests/feature_engineering` folder.
...
1. Implement your feature module, address TODOs in `mlops-poc/feature_engineering/features` and create unit test in `mlops-poc/tests/feature_engineering`
2. Update `mlops-poc/terraform/staging/write-feature-table-job.tf` and `mlops-poc/terraform/prod/write-feature-table-job.tf`. Fill in parameters for `GenerateAndWriteFeatures` job.
3. Update training data path in `mlops-poc/terraform/staging/training-job.tf` and `mlops-poc/terraform/prod/training-job.tf`
